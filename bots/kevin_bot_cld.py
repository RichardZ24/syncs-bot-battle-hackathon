...
from lib.config.player import EAT_SIZE_RATIO, SPLIT_MIN_MASS, SPLIT_COOLDOWN_FRAMES

# --- kevin_bot v2.0 ---
# Engine Constants
MAP_MAX = ARENA_SIZE          # was hardcoded 60.0, now tracks real config
EAT_RATIO = EAT_SIZE_RATIO     # was hardcoded 1.2, now tracks real config
# our own "worth it to hunt" heuristic, not an engine constant
SPLIT_RATIO = 2.4
# SPLIT_EJECT_SPEED / (1 - SPLIT_EJECT_DRAG) = 1.6/0.18
SPLIT_JUMP = 8.88
MAX_BLOBS = 16
BAIT_RANGE = 20.0
...
class BotMemory:
    def __init__(self):
        ...
        self.last_split_tick = -10_000  # local cooldown tracker for our own splits
...
def calculate_move(query: QueryMovePlayer, memory: BotMemory) -> MovePlayer:
    ...
    enemy_data = []
    # FIX: engine's real split floor is SPLIT_MIN_MASS (2.0, radius ~1.41), not the
    # old hardcoded 4.0 (radius 2.0). That magic number was silently cutting our
    # splitting window roughly in half for most of the early/mid game.
    can_physically_split = my_blob_count < MAX_BLOBS and my_mass > SPLIT_MIN_MASS
    ...
    for i in range(NUM_RAYS):
        ...
        # FIX: viruses now only pop on center-overlap (per changelog), so the
        # relevant danger corridor is our own radius, not "my_r + v.radius".
        # The old buffer made us flinch away from viruses we could safely skirt.
            ...
            for v in query.visible_viruses:
                if my_mass > (v.radius ** 2) * EAT_RATIO:
                    ...
                    if 0 < along_dist < 30.0:
                        ...
                        if perp_dist < (my_r + 1.0):
                            score -= 500000.0 / max(0.1, along_dist ** 2)

        # NEW: virus-bait. If we're small enough to pass a virus untouched but a
        # closing predator is big enough to get popped by it, nudge our escape
        # route to graze the virus rather than run a straight line. A predator
        # tailing our exact path eats the virus and gets shattered; we don't.
        if my_r > 1.0 and query.visible_viruses:
            for v in query.visible_viruses:
                if my_mass > (v.radius ** 2) * EAT_RATIO:
                    continue
                vdx, vdy = v.pos[0] - my_x, v.pos[1] - my_y
                along_dist = rx * vdx + ry * vdy
                if 0 < along_dist < BAIT_RANGE:
                    perp_dist = abs(rx * vdy - ry * vdx)
                    if perp_dist < (my_r + 1.5):
                        for e in enemy_data:
                            if e['is_threat'] and not e['cornered'] and e['closure_rate'] < -0.02:
                                pursuer_mass = player_masses.get(
                                    e['pid'], e['r'] ** 2)
                                if pursuer_mass > (v.radius ** 2) * EAT_RATIO:
                                    score += 400.0 / max(1.0, along_dist)
                                    break

        if query.visible_food:
            eat_radius = my_r + 0.15
            # NEW: soft falloff instead of a hard cutoff at eat_radius. The old
            # corridor was a strict on/off test per ray (32 rays = 11.25 deg steps),
            # so food sitting just off-axis scored zero even though the sweep
            # would likely still catch it. Widening with a taper rewards rays
            # that sweep through denser food, not just perfectly-aligned pellets.
            wide_radius = eat_radius + 1.5
            for f in query.visible_food:
                ...
                if 0 < along_dist < 25.0:
                    ...
                    if perp_dist < wide_radius:
                        accuracy_mult = 1.0 - (perp_dist / wide_radius)
                        ...
                        if query.visible_viruses:
                            for v in query.visible_viruses:
                                if halved_mass > (v.radius ** 2) * EAT_RATIO:
                                    ...
                                    vdot = best_ray[0] * \
                                        vdx + best_ray[1] * vdy
                                    # FIX: same center-overlap correction as above.
                                    if 0 < vdot < (halved_r + SPLIT_JUMP):
                                        perp = abs(
                                            best_ray[0] * vdy - best_ray[1] * vdx)
                                        if perp < (halved_r + 0.5):
                                            virus_blocked = True
                                            break
                        if not virus_blocked:
                            do_split = True
                            memory.last_split_tick = memory.tick
                            break

    # NEW: emergency escape-split. SPLIT_COOLDOWN_FRAMES is only 18 ticks (1.8s)
    # and merge is fast once off cooldown, so splitting to flee is cheap: the
    # eject burst (SPLIT_EJECT_SPEED=1.6, decaying) usually outruns a big slow
    # blob's cruise speed, and the resulting half-radius blobs are themselves
    # faster (speed scales inversely with radius). If we're caught anyway we
    # only lose one half instead of the whole blob; if we escape we remerge.
    if can_physically_split and not do_split:
        ticks_since_split = memory.tick - memory.last_split_tick
        if ticks_since_split >= SPLIT_COOLDOWN_FRAMES:
            threats_now = [e for e in enemy_data if e['is_threat']]
            if threats_now:
                worst = min(
                    threats_now, key=lambda e: e['dist'] - (my_r + e['r']))
                gap = worst['dist'] - (my_r + worst['r'])
                if gap < SPLIT_JUMP * 0.7 and worst['closure_rate'] < -0.02:
                    do_split = True
                    memory.last_split_tick = memory.tick
